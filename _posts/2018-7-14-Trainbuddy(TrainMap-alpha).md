...
so I got a full time job and didn't have much time to work on the app. Then the back-end service I used got discontinued. You may too remember about Parse(a facebook service). I didn't have time to port the app to another backend so I removed the train map feature(nobody used that in that either). So the train schedule app worked fine for some time. Then the API I used(provided by ICTA) closed the open access. So the schedule function stopped working too. That was the reason for app saying you are not connected to internet even though you are connected. Because it couldn't communicate with the API.

So in today's release I stripped all the schedule features and brought back the train map features. I hope to gradually increase the user base and make this service stable by the time. Today's release is the first step in that direction.
